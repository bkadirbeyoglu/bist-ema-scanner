...
After the BIST close, run `bist_ema_scanner.py`. It walks every stock in the chosen index (XU030 (BIST 30), XU100 by default, or XU500), fetches the last 6 months of daily candles from Yahoo Finance, computes EMA-20 and EMA-50, and prints the stocks where today's session matches one of two breakout patterns. Hits are appended to a CSV log, and the outcome of every past hit (1-10 day follow-up returns, intraday range, volume continuity, market-relative performance, trend age, liquidity profile) is filled in automatically as more sessions pass.
...
> **Note on BIST 30.** `-i xu030` scans only the 30 BIST 30 constituents (rebalanced quarterly, like the other indices — refresh after Jan/Apr/Jul/Sep). The market-relative reference stays **XU100** for every universe, so XU030 signals remain comparable to XU100/XU500 ones. XU030 keeps its own separate log/outcome files, so a fresh forward track starts cleanly.
...
```
xu030.csv                  ← ticker list (regenerated by update_index.py)
xu100.csv
xu500.csv

signals_log_xu030.csv      ← every signal ever fired
signals_log_xu100.csv
signals_log_xu500.csv

outcomes_xu030.csv         ← post-signal d1..d10 returns + intraday + market-rel
outcomes_xu100.csv
outcomes_xu500.csv
```
...
| `atr14`, `atr_pct`, `break_atr`, `ema_gap_atr` | ATR(14) + ATR-normalised entry geometry (v1.14): Wilder ATR (absolute), ATR as % of close, and the extension above the upper EMA / the EMA20-EMA50 gap expressed in ATRs |

Duplicate-protected on `(signal_date, ticker)` (v1.15) — a signal already in the log is never re-appended, so the scanner is safe to re-run any number of times, the same day or a later one. (Before v1.15 the dedup key also included `scan_date`, so re-running on a later day for an unchanged session created duplicate rows; clean any old logs once with `dedup_signals.py`.)
...
**ATR-normalised entry geometry (v1.14).** `atr14` is the Wilder-smoothed Average True Range over 14 days (absolute price units); `atr_pct` is `atr14 / close × 100`, the stock's typical daily swing as a percentage. `break_atr` and `ema_gap_atr` re-express the existing `break_pct` and `ema_gap_pct` in ATR units — extension above the upper EMA, and the EMA20-EMA50 gap, measured in ATRs. The point is volatility comparability: on raw entry-knowable geometry, `break_pct` was non-monotonic against forward return (the 3-5% bucket was weakest, >5% strongest) because a fixed % means very different things on a low-vol versus high-vol name; "how many ATRs" should discriminate where the raw % could not. Mirrored in both logs, seeded once at signal time. Slope-in-ATR is not stored separately because it is derivable as `ema20_slope / atr_pct`. `atr14` is also the intended basis for a future ATR-scaled stop.
...
After a few weeks, this file is a goldmine for analysis: open it in Excel or pandas, pivot by `trigger`, by `vol_ratio` bucket, by `break_pct` quintile, by gap size (`d1_open - signal_close`), by `close_in_range` position, by `day_of_week`, by `days_above_ema20` band, by `ema20_slope` sign/magnitude, by `break_atr` / `atr_pct` (volatility-normalised geometry), by `avg_tl_volume_20d` decile, by KAP activity (`kap_signal_day`, `kap_category_breakdown`, joined from signals_log), or by market-relative performance at any horizon from d1 through d5, and see which conditions actually predict positive subsequent returns.
...
Both log files automatically migrate to the current schema when the scanner runs. If new columns have been added between versions (e.g. `signal_close_in_range` in v1.6, `split_suspect` in v1.7, `days_above_ema20` / `days_above_ema50` in v1.9, `avg_tl_volume_20d` in v1.10, `xu100_d2_close` / `xu100_d3_close` / `xu100_d4_close` / `xu100_d5_close` in v1.11, the `kap_*` columns in v1.12, and `ema20_slope` / `ema50_slope` in v1.13, and `atr14` / `atr_pct` / `break_atr` / `ema_gap_atr` in v1.14), the scanner prints a "Migrating … adding columns […]" line on the next run and rewrites the file with the new headers; existing rows get empty values for the new columns. Pre-migration data is preserved untouched. For the v1.11 columns specifically, the new market-reference cells get populated progressively as the scanner runs on subsequent trading days — every signal whose d2..d5 dates have already passed at the time of the next run gets filled in a single pass. The v1.12, v1.13 and v1.14 columns are signal-time information, seeded only on newly detected signals; pre-existing rows keep empty values (a one-off backfill from yfinance history can populate them retroactively if needed).
...
│
├── xu030.csv                       # Ticker lists (generated)
├── xu100.csv
├── xu500.csv
