...
class TestOrder:
    ...
    def test_create_market_order(self):
        ...
        order = Order.create_market_order(
            symbol=symbol, quantity=100, side=OrderSide.BUY
        )
    ...
    def test_create_limit_order(self):
        ...
        order = Order.create_limit_order(
            symbol=symbol, quantity=50, side=OrderSide.SELL, limit_price=limit_price
        )
    ...
    def test_cannot_fill_before_submit(self):
        ...
        with pytest.raises(
            ValueError, match=f"Cannot fill order in {OrderStatus.PENDING} status"
        ):
